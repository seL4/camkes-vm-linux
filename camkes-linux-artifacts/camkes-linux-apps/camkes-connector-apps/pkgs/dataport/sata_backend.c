...
unsigned int handle_tx(int client_idx)
{
    ...
    if (offset != handlers[client_idx]->value * VIRTIO_BLK_DISK_BLK_SIZE) {
        printf("weird things happened in lseek, expected offset: %ld, returned offset: %ld",
               handlers[client_idx]->value * VIRTIO_BLK_DISK_BLK_SIZE, offset);
        return 0;
    }

    ssize_t byte_written = write(connections[client_idx].blocks_fd, handlers[client_idx]->buffer, handlers[client_idx]->len);
    if (byte_written < 0) {
...
unsigned int handle_rx(int client_idx)
{
    ...
    ssize_t byte_read = read(connections[client_idx].blocks_fd, handlers[client_idx]->buffer, handlers[client_idx]->len);
    if (byte_read < 0) {
...
int main(int argc, char *argv[])
{
    ...
    while (num_conns) {
        int num_victims = poll(fds, num_conns, -1);
