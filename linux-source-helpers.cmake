...
function(GetDefaultLinuxMd5 md5)
    set(${md5}
        "5230b0185b5f4916feab86c450207606"
        PARENT_SCOPE
    )
...
function(DownloadLinux linux_major linux_minor linux_md5 linux_out_dir linux_out_target)
    ...
    file(
        WRITE ${CMAKE_CURRENT_BINARY_DIR}/download_linux.cmake
    ...
    add_custom_command(
        OUTPUT out/${linux_archive}
        COMMAND ${CMAKE_COMMAND} -P ${CMAKE_CURRENT_BINARY_DIR}/download_linux.cmake
        VERBATIM
    )
    add_custom_target(
        download_${linux_out_target} DEPENDS "${CMAKE_CURRENT_BINARY_DIR}/out/${linux_archive}"
    )
    ...
    add_custom_command(
        OUTPUT out/${linux_dir}/.cmake_extracted
    ...
    add_custom_target(
        extract_${linux_out_target}
        DEPENDS "${CMAKE_CURRENT_BINARY_DIR}/out/${linux_dir}/.cmake_extracted"
    )
    # Add clean target for extracted linux directory.
    add_custom_target(
        clean-${linux_out_target} COMMAND bash -c
                                          "rm -r ${CMAKE_CURRENT_BINARY_DIR}/out/${linux_dir}"
    )
    # Add linux out target
    add_custom_target(${linux_out_target} DEPENDS extract_${linux_out_target})
    set_property(
        TARGET ${linux_out_target} PROPERTY EXTRACT_DIR
                                            "${CMAKE_CURRENT_BINARY_DIR}/out/${linux_dir}"
    )
...
function(ConfigureLinux linux_dir linux_config_location linux_symvers_location
         configure_linux_target
)
    # Get any existing dependencies for configuring linux
    cmake_parse_arguments(PARSE_ARGV 4 CONFIGURE_LINUX "" "ARCH;CROSS_COMPILE" "DEPENDS")
    if(NOT "${CONFIGURE_LINUX_UNPARSED_ARGUMENTS}" STREQUAL "")
        message(
            FATAL_ERROR "Unknown arguments to ConfigureLinux: ${CONFIGURE_LINUX_UNPARSED_ARGUMENTS}"
        )
    endif()
    set(configure_flags "")
    ...
    add_custom_command(
        OUTPUT ${linux_dir}/.config ${linux_dir}/Module.symvers
    ...
    add_custom_target(
        ${configure_linux_target}_copy_configs DEPENDS "${linux_dir}/.config"
                                                       "${linux_dir}/Module.symvers"
    )
    # Prepare/Configure Linux Build Directory
    add_custom_command(
        OUTPUT ${linux_dir}/.config.old
